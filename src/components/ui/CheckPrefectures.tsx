    ...
    <>
      <ul css={grid}>
        {prefecture.map((prefecture) => (
          <li key={prefecture.prefCode} css={gridItem}>
            <label css={inputButton}>
              <input type="checkbox" name="" value={prefecture.prefCode} onChange={addPrefectureCodeHandler} />
              <span css={text}>{prefecture.prefName}</span>
            </label>
          </li>
        ))}
      </ul>
